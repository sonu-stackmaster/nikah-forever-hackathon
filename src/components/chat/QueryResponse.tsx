...
export function QueryResponse({ query }: QueryResponseProps) {
  ...
  const renderVisualization = () => {
    ...
    switch (query.result.visualization_type) {
      ...
      case 'table':
        return (
          <div className="my-4 overflow-hidden rounded-xl border border-pink-100 shadow-sm">
            <DataTable data={query.result.data} />
          </div>
        )
      case 'chart':
        return (
          <div className="my-4 p-4 bg-white rounded-xl border border-pink-100 shadow-sm">
            <ChartRenderer 
  ...
  return (
    <div className="flex flex-col space-y-6">
      ...
      <div className="flex justify-end w-full">
        <div className="max-w-[75%] bg-gradient-to-r from-pink-500 to-rose-500 text-white px-5 py-3 rounded-2xl rounded-tr-sm shadow-sm">
      ...
      <div className="flex items-start space-x-3 w-full">
        {/* Avatar */}
        <div className="flex-shrink-0 p-2 bg-gradient-to-tr from-pink-500 to-rose-500 rounded-xl shadow-md">
          <Brain className="h-5 w-5 text-white" />
        </div>

        {/* Response Body */}
        <div className="flex-1 bg-white border border-pink-100/80 rounded-2xl rounded-tl-sm p-5 shadow-sm max-w-full overflow-hidden">
          ...
          {/* database response content (if SQL was executed) */}
          {!isChatOnly && query.sql && (
            <div className="mt-4 pt-4 border-t border-pink-50 space-y-4">
              {/* Toolbar */}
              <div className="flex items-center justify-between text-xs text-gray-500">
                <div className="flex items-center space-x-3">
                  <Button
                    ...
                    className="h-8 border-pink-100 hover:bg-pink-50 hover:text-pink-600 text-gray-600 flex items-center gap-1.5"
                  >
                  ...
                  {showSQL && (
                    <Button
                      ...
                      size="sm"
                      onClick={copySQL}
                      className="h-8 hover:bg-pink-50 hover:text-pink-600 text-gray-500 flex items-center gap-1"
                    >
                      {copied ? (
                        <>
              ...
              {/* SQL Display */}
              {showSQL && (
                <div className="relative rounded-lg overflow-hidden border border-pink-100/60 bg-gray-900 text-gray-100 font-mono text-xs md:text-sm p-4">
          ...
          {/* Error Display */}
          {query.error && (
            <div className="mt-4 bg-red-50 border border-red-100 p-4 rounded-xl flex items-start space-x-3">
